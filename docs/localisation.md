...
本地化（Localisation/Localization）与翻译（Translation）相比，除了要完成基本的翻译以外，还要使翻译后的词语/语句符合当地的语言习惯。不过对于模组作者来说，如果不考虑发布多语言的Mod，则可以不考虑翻译的问题。

在群星游戏中，本地化是用于将代码中的一些`Key`转换为对应语言的文件。

## 认识本地化文件

对于任意的本地化文件，它总是满足以下几个条件：
...
> 由于本教程面向中文Modder，因此以下均以**简体中文**语言为例。
...
> ```
>
> 这个条目在`:`后多了一个数字`0`，该数字对本地化无影响，推测是P社用于标记该条目被修改次数用的，在3.9版本更新后，几乎所有的`:0`均被修改为了`:`。

## 一些注意事项

+ `\t`用于表示四个空格的缩进
+ `\n`用于表示换行
+ 如果你需要在本地化中书写英文双引号`"`，则你需要使用`\"`替代
...
> `§`可在Windows上使用键盘快捷键`ALT`+小键盘`0167`打出来。
...
> **注意：**
>
> + 上方表格中所展示颜色的背景色是为了显示清除颜色，颜色代码并不会改变文本的背景色
>
...
以上的几种格式化方法可以混合使用，且顺序是无关紧要的，例如：`$VALUE|0=-%$`，当`VALUE`为`1`时，显示为<font style="color:#29E126;background-color:#000000">+100%</font>，当`VALUE`为`-1`时，显示为<font style="color:#FC5646;background-color:#000000">-100%</font>，当不指定小数位数时，默认显示2位小数。
...
> `£`可在Windows上使用键盘快捷键`ALT`+小键盘`0163`打出来。
...
支架命令可分为硬代码规定的默认命令和Modder自定义的`scripted_loc`。
...
由于原版硬代码规定的默认命令数量庞大，以下仅列出部分常用的命令：
...
脚本本地化允许创建你自己的动态支架命令，其定义于`common/scripted_loc`文件夹，基本语法如下：

```pdx
defined_text = {
	name = GetExampleText 					# 在本地化中使用的命名名称，本例为：[Root.GetExampleText]
	random = no								# 若设置为no，当多个文本都满足条件时，会选择权重最高的那个，若权重相同，则选择第一个
											# 若设置为yes（默认），当多个文本都满足条件时，会从满足条件的文本中基于权重随机选择一个
	text = {
		localization_key = text_something	# 若条件满足时显示的文本
		trigger = {							# 显示该文本所需要的条件，留空默认为永远满足
		}
		weight = {							# 权重，当多个文本满足条件时用来决定哪个文本会被使用
			weight = 1 						# 权重默认值为1
			modifier = {
				factor = 2
			}
	...
	default = default_text 					# 默认文本，当其余所有文本的条件均不满足时才会显示这个文本
}
```

你也可以用`scripted_loc`来显示`script_value`的值：
...
当使用时（`e.g. [Root.scripted_value_test]`），会显示在该域下`desired_science_ships_system_mult`这个`script_value`计算所得的值。
